function [net,stats] = cnn_train_dag_gd2_info(net, imdb, getBatch, varargin)
...
for t=1:opts.batchSize:numel(subset)
  now_batch = fix((t-1)/opts.batchSize)+1;
  %---------------------------------------GD----------------------------
  
  if(mod(now_batch,2)==0)
      fprintf('updating G\n');
      opts.derOutputs = {'Dobjective', 0,'Gobjective', 1,'GDobjective',.1} ;
      opts.flag1 = true;
      opts.flag2 = false;
  else
      fprintf('updating D\n');
      opts.derOutputs = {'Dobjective', .1,'Gobjective', 0,'GDobjective',.1} ;
      opts.flag1 = false;
      opts.flag2 = true;
  end
  %}
  ...
  switch net.params(p).trainMethod
    ...
    case 'average' % mainly for batch normalization
      ...
      if( opts.flag1)
          ...
          if (net.params(p).name(1)=='G')  %keep G update D
            thisLR = 0;
          end
      end
      %}
    ...
    case 'rmsprop'
     ...
     case 'adam'
      ...
      beta1 = opts.beta1;
      beta2 = opts.beta2;
      thisLR = state.learningRate * net.params(p).learningRate * sqrt(1-beta2^2)/(1-beta1^state.epoch);
