function eval_g(obj, inputs, derOutputs)
...
for l = obj.executionOrder
  ...
  affindex = obj.getVarIndex('aff');
  if(~isnan(affindex))
    aff = gather(obj.vars(affindex).value);
    meanaff = mean(aff,4);
    if(~isempty(meanaff))
        fprintf('meanaff%.3f,%.3f,%.3f,%.3f,%.3f,%.3f',meanaff(1),meanaff(2),meanaff(3),meanaff(4),meanaff(5),meanaff(6));
    end
  end
% -------------------------------------------------------------------------
% Backward pass
...
for l = fliplr(obj.executionOrder)
  ...
  if(strcmp(obj.layers(l).name,'GDloss'))
     derInputs = obj.vars(obj.getVarIndex('prediction')).der;
     half = round(numel(derInputs)/4);          
     derInputs(:,:,:,half+1:end)=0;
     obj.vars(obj.getVarIndex('prediction')).der = derInputs;
  end
