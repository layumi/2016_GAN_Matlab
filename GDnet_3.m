function net = GDnet_3()
net = dagnn.DagNN();
GreluBlock = dagnn.ReLU('leak',0);
reluBlock = dagnn.ReLU('leak',0.2);
SigmoidBlock = dagnn.Sigmoid();
%G
conv1Block = dagnn.Conv('size',[1 1 100 500],'hasBias',true,'stride',[1,1],'pad',[0,0,0,0]);
...
net.addLayer('GDloss',lossBlock,{'prediction','label'},'Dobjective');
